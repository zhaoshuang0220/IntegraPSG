...
def xyxy_to_x1y1wh(xyxy: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
...
def xcycwh_to_xyxy(xywh: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
...
def xyxy_to_xcycwh(xyxy: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
...
def process_vg_bbox(
    og_height: int, og_width: int, bbox: Tuple[int, int, int, int], resize: int = 1024
) -> Tuple[int, int, int, int]:
...
def process_vg_150_to_detectron(
    ...
    for name, (start_idx, end_idx) in zip(
        ...
        for img_idx in tqdm(range(start_idx, end_idx)):
            ...
            for i, box_idx in enumerate(range(first_box_idx, last_box_idx + 1)):
                anno = {}
                ...
                for old_attr_id in attrs:
                    if old_attr_id != 0:
                        attr_name = vg_metadata["idx_to_attribute"][str(old_attr_id)]
...
def process_coco_panoptic_to_detectron(
    ...
    stuff_cats = sorted([cat["name"] for cat in cats_dict if not cat["isthing"]])
    ...
    new_thing_cat_id_map = {
        cat["id"]: thing_cats.index(cat["name"]) for cat in cats_dict if cat["isthing"]
    }
