...
from IntegraPSG.datasets import build_dataset
from IntegraPSG.datasets.psg import PanopticSceneGraphDataset
...
def main():
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            if isinstance(dataset, PanopticSceneGraphDataset):
                metric = dataset.evaluate(outputs, **eval_kwargs)
            else:
                raise RuntimeError(
                    f" {type(dataset)}，请检查 config 文件 data.test 设置！"
                )
            print(metric)
