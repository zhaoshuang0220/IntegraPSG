...
from .sgg_metrics import (SGAccumulateRecall, SGMeanRecall,
                          SGNoGraphConstraintRecall, SGObjectIOU,
                          SGPairAccuracy, SGRecall, SGZeroShotRecall)
...
def vg_evaluation_single(
    ...
    eval_recall = SGRecall(
        result_dict, nogc_result_dict, nogc_thres_num, detection_method=detection_method
    )
    eval_recall.register_container(mode)
    evaluator["eval_recall"] = eval_recall

    # used by https://github.com/NVIDIA/ContrastiveLosses4VRD for sgcls and predcls
    eval_pair_accuracy = SGPairAccuracy(
        result_dict, nogc_result_dict, nogc_thres_num, detection_method
    )
    ...
    eval_iou = SGObjectIOU(
        result_dict, nogc_result_dict, nogc_thres_num, detection_method=detection_method
    )
    eval_iou.register_container(mode)
    evaluator["eval_iou"] = eval_iou
    ...
    for groundtruth, prediction in zip(groundtruths, predictions):
        ...
        evaluate_relation_of_one_image(
            groundtruth, prediction, global_container, evaluator
        )
        pbar.update()
    ...
    result_str += eval_iou.generate_print_string(mode)
    result_str += eval_mean_recall.generate_print_string(mode, predicate_freq)
    if mode != "sgdet" and mode != "pairdet":
...
def evaluate_relation_of_one_image(
    ...
    #local_container["gt_masks"] = getattr(groundtruth, "masks", None)
    #local_container["pred_masks"] = getattr(prediction, "masks", None)
    ...
    if mode == "predcls":
        ...
        if (
            local_container["gt_boxes"].shape[0]
            != local_container["pred_boxes"].shape[0]
        ):
            print("Num of GT boxes is not matching with num of pred boxes in SGCLS")
    elif mode == "sgdet" or mode == "phrdet" or mode == "pairdet":
        pass
    else:
    ...
    # GT Pair Accuracy
    evaluator["eval_pair_accuracy"].calculate_recall(
        global_container, local_container, mode
    )
    # Mean Recall
    ...
    # iou
    evaluator["eval_iou"].calculate_iou(global_container, local_container, mode)
...
def generate_attributes_target(attributes, num_attributes):
    ...
    attribute_targets = torch.zeros(
        (num_obj, num_attributes), device=attributes.device
    ).float()
