...
from IntegraPSG.evaluation import sgg_evaluation
        ...
        if len(sg_metrics) > 0:
            ...
            if not hasattr(self, "test_gt_results"):
                ...
                for i in range(len(self)):
                    ...
                    prog_bar.update()    
                print(f"groundtruth[{0}] attrs:", dir(gt_results[-1]))  # 新增此行6.29
                print("\n")
                print(f"groundtruth[{0}] keys:", list(vars(gt_results[-1]).keys()))
                print("\n")
