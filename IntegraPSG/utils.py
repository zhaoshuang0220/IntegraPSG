...
def masks_to_boxes(masks: torch.Tensor) -> torch.Tensor:
    ...
    #         bounding_boxes[index] = torch.zeros((4), device=masks.device, dtype=torch.float)
    #     else:
    #         y, x = torch.where(mask != 0)
    #         bounding_boxes[index, 0] = torch.min(x) / w
    #         bounding_boxes[index, 1] = torch.min(y) / h
    #         bounding_boxes[index, 2] = torch.max(x) / w
    #         bounding_boxes[index, 3] = torch.max(y) / h
    #         bounding_boxes[index] = box_convert(bounding_boxes[index], 'xyxy', 'cxcywh')

    # return bounding_boxes # (cxcywh, normalized)
    _, h, w = masks.shape
...
def multiclass_nms_alt(
    ...
    if return_dist:
        ...
        return torch.cat([bboxes, scores[:, None]], 1), (labels.view(-1), score_dists)
    else:
...
def extract_depth_features(depth_map, masks):
    ...
    for i in range(masks.shape[0]):
        ...
        if not mask.any():
            feats.append(torch.zeros(3, device=depth_map.device))
            continue


        valid_depth = (depth_map > 0) & (depth_map < 1e4)
        valid_mask = (mask > 0) & valid_depth
        ...
        region = depth_map[valid_mask].float()
        ...
        try:
            if region.numel() > 1:
                mean = region.mean()
                std = region.std()
                valid = torch.abs(region - mean) < 3 * std
                if valid.any():
                    region = region[valid]

            mean_val = region.mean().clamp(-100, 100)
            std_val = region.std().clamp(0, 100) if region.numel() > 1 else torch.tensor(0.0).to(region.device)
            min_val = region.min().clamp(0, 100)

            feat = torch.stack([mean_val, std_val, min_val])

            if torch.isnan(feat).any() or torch.isinf(feat).any():
                feat = torch.zeros(3, device=depth_map.device)
