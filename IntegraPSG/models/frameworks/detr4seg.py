...
class DETR4seg(SingleStageDetector):
    def __init__(
        ...
        super(DETR4seg, self).__init__(
            backbone, None, bbox_head, train_cfg, test_cfg, pretrained, init_cfg
        )
    ...
    def forward_train(
        self, img, img_metas, gt_bboxes, gt_labels, gt_masks, gt_bboxes_ignore=None
    ):
        """
        Args:
        ...
        for each in gt_masks:
            mask = torch.tensor(
                each.resize((H // 2, W // 2), interpolation="bilinear").to_ndarray(),
                device=x[0].device,
            )
            _, h, w = mask.shape
            # padding = (
