...
from IntegraPSG.models.relation_heads.approaches import Result
from IntegraPSG.utils import adjust_text_color, draw_text, get_colormap
...
class SceneGraphPanopticFPN(PanopticFPN):
    ...
    def simple_test_sg_bboxes(self, x, img_metas, proposals=None, rescale=False):
        ...
        for bbox, score in zip(bboxes, scores):
            det_bbox, det_label = multiclass_nms(
                bbox, score, pan_cfg.score_thr, pan_cfg.nms, pan_cfg.max_per_img
            )
        ...
        for i in range(len(det_bboxes)):
            ...
            labels = np.array([id % INSTANCE_OFFSET for id in ids], dtype=np.int64) + 1
        ...
        **kwargs,
        ...
        if self.with_relation:
            ...
            for idx, param in enumerate(param_need_filter):
                if param_need_filter[idx]:
                    param_need_filter[idx] = [
                        x for i, x in enumerate(param) if i not in idxes_to_filter
                    ]
            ...
            for idx in range(len(x)):
                filtered_x.append(
                    torch.stack(
                        [e for i, e in enumerate(x[idx]) if i not in idxes_to_filter]
                    )
                )
            x = filtered_x
            ...
            det_result = self.relation_head(x, img_metas, det_result, gt_result)
    ...
    def forward_test(self, imgs, img_metas, **kwargs):
        ...
        # if relation_mode:
        assert num_augs == 1
        return self.relation_simple_test(
            imgs[0], img_metas[0], key_first=key_first, **kwargs
        )
    ...
    def detector_simple_test(
        ...
        # NOTE: Sgcls should not be performed
            ...
            if not is_testing:  # excluding the testing phase
                det_results = self.simple_test_sg_bboxes(x, img_meta, rescale=rescale)
                ...
                mask_size = (img_meta[0]["ori_shape"][0], img_meta[0]["ori_shape"][1])
                pan_seg_masks = (
                    F.interpolate(
                        torch.Tensor(pan_seg_masks[0]).unsqueeze(1), size=mask_size
                    )
                    .squeeze(1)
                    .bool()
                )
                ...
                det_results_for_pan = self.simple_test_sg_bboxes(
                    x, img_meta, rescale=True
                )
            ...
            det_dists = [
                F.one_hot(det_label, num_classes=self.num_classes + 1).to(det_bboxes[0])
            ...
            det_bboxes = [
                torch.cat([b, b.new_ones(len(b), 1)], dim=-1) for b in det_bboxes
            ]
    ...
    def detector_simple_test_det_bbox(self, x, img_meta, proposals=None, rescale=False):
    ...
    def relation_simple_test(
        ...
        det_result = self.relation_head(
            x, img_meta, det_result, is_testing=True, ignore_classes=ignore_classes
        )
        """
        ...
        det_result = self.relation_head.get_result(
            det_result, scale_factor, rescale=rescale, key_first=key_first
        )
