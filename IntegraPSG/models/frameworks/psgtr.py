...
def triplet2Result(triplets, use_mask, eval_mask_rels=False):
    if use_mask:
        # bboxes, labels, rel_pairs, masks, pan_seg, r_scores, r_labels, r_dists, sub_pos, obj_pos \
        (
            bboxes,
            labels,
            rel_pairs,
        ...
        sub_pos = sub_pos.detach().cpu().numpy()
...
class PSGTr(SingleStageDetector):
    ...
    def forward_train(
        ...
        losses = self.bbox_head.forward_train(
            x, img_metas, gt_rels, gt_bboxes, gt_labels, gt_masks, gt_bboxes_ignore, depth=depth#6月16号新增的
        )
        return losses

    def simple_test(self, img, img_metas, rescale=False, depth=None):
        feat = self.extract_feat(img)
        results_list = self.bbox_head.simple_test_bboxes(
            feat, img_metas, rescale=rescale, depth=depth
        )
        sg_results = [
