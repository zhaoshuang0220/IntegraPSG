...
# from IntegraPSG.models.frameworks.unet import UNet
from IntegraPSG.models.frameworks.cnn_factory import creat_cnn


@HEADS.register_module()
class CrossHeadDirect(AnchorFreeHead):
    def __init__(
        self,
        num_classes,
        in_channels,
        ...
        **kwargs,
        ...
        # self.update_importance = UNet(in_channels=1, out_channels=1)
        ...
        self.pair_embed = nn.Embedding(self.num_queries * 2, feat_channels)
        # from low resolution to high resolution
        ...
        self.sub_cls_embed = copy.deepcopy(self.cls_embed)
        self.obj_cls_embed = copy.deepcopy(self.cls_embed)

        self.sub_mask_embed = copy.deepcopy(self.mask_embed)
        self.obj_mask_embed = copy.deepcopy(self.mask_embed)
    ...
    def _init_layers(self):
        ...
        self.rel_cls_mlp = nn.Sequential(
            Linear(self.embed_dims, self.embed_dims),
            nn.ReLU(inplace=True),
            Linear(self.embed_dims, self.embed_dims),
            nn.ReLU(inplace=True),
    ...
    def mask_head(self, decoder_out, mask_feature):
        """Forward for head part which is called after every decoder layer.
    ...
    def forward(self, feats, img_metas):
        ...
        rel_embed = self.rel_cls_mlp(rel_query_feat)

        # sub_cls_pred, sub_mask_pred, _ = self.forward_head(
        #     sub_embed,
        #     mask_features,
        #     multi_scale_memorys[0].shape[-2:],
        # )
        # obj_cls_pred, obj_mask_pred, _ = self.forward_head(
        #     obj_embed,
        #     mask_features,
        #     multi_scale_memorys[0].shape[-2:],
        # )

        sub_out = self.transformer_decoder.post_norm(sub_embed).transpose(0, 1)
        obj_out = self.transformer_decoder.post_norm(obj_embed).transpose(0, 1)
        # shape (num_queries, batch_size, c)
        sub_cls_pred = self.sub_cls_embed(sub_out)
        obj_cls_pred = self.obj_cls_embed(obj_out)
        # shape (num_queries, batch_size, c)
        sub_mask_embed = self.sub_mask_embed(sub_out)
        obj_mask_embed = self.obj_mask_embed(sub_out)
        # shape (num_queries, batch_size, h, w)
        sub_mask_pred = torch.einsum("bqc,bchw->bqhw", sub_mask_embed, mask_features)
        obj_mask_pred = torch.einsum("bqc,bchw->bqhw", obj_mask_embed, mask_features)

        rel_cls_pred = self.rel_cls_embed(rel_embed).transpose(0, 1)
        # just try.
        ...
        # sub_pos = torch.randint(0, 100, (2, 100)).to(importance).to(torch.int64)
        # obj_pos = torch.randint(0, 100, (2, 100)).to(importance).to(torch.int64)
        ...
        # pair_feat = self.pair_embed.weight.unsqueeze(1).repeat(
        #     (1, batch_size, 1)
        # )
        ...
        #     ),
        # )
        importance = torch.ones(batch_size, 100, 100).to(sub_mask_pred)

        all_cls_scores = dict(
            sub=sub_cls_pred,  # (b, 100, 134)
            obj=obj_cls_pred,  # (b, 100, 134)
            cls=cls_pred,  # (b, 100, 134)
            rel=rel_cls_pred,  # (b, 100, 56)
            importance=importance,  # (b, 100, 100)
        )
        all_mask_preds = dict(
            mask=mask_pred,  # (b,100,h,w)
            sub_seg=sub_mask_pred,  # (b,100,h,w)
            obj_seg=obj_mask_pred,  # same
        )
