...
class PostProcessor(nn.Module):
    ...
    def forward(self, det_result, key_first=False):
        ...
        if det_result.refine_scores is None:
            return det_result
        relation_logits, finetune_obj_logits = (
        ...
        for i, (rel_logit, obj_logit, rel_pair_idx, bbox) in enumerate(
            zip(relation_logits, finetune_obj_logits, rel_pair_idxes, det_result.bboxes)
        ):
...
class DemoPostProcessor(object):
    ...
    def filter_nonoverlap_rels(self, det_result, must_overlap_predicates=None):
        ...
        for idx, (pair, predicate) in enumerate(zip(rel_pair_idxes, rel_labels)):
            ...
            iou = ious[subj, obj]
            if (
                must_overlap_predicates is not None
                and predicate in must_overlap_predicates
                and iou <= 0
            ):
                continue
            new_rel_pair_idxes.append(pair)
    ...
    def forward(
        ...
        keep_obj_ids = np.where(np.isin(det_result.refine_labels, ignore_classes) == 0)[
            0
        ]
        ...
        score_keep_obj_ids = np.where(det_result.refine_bboxes[:, -1] >= object_thres)[
            0
        ]
        ...
        det_result = self.filter_duplicate_triplets(
            det_result, vocab_objects, vocab_predicates
        )
        ...
        rel_num = (
            max_rel_num
            if max_rel_num is not None
            else int(num_obj * (num_obj - 1) / 2 - num_obj)
        )
...
def group_regions(result, prior_pairs, thres=0.9):
    ...
    for boxes, labels in zip(bboxes, obj_labels):
        ...
        if num_obj == 0:
            ...
            continue
        box_areas = (boxes_np[:, 2] - boxes_np[:, 0] + 1) * (
                    ...
                    if subj_area > obj_area:
                        if intersect[j, i] > thres:
                            if j in region_group:
                                region_group[i] = list(
                                    set(region_group[i] + region_group[j] + [j])
                                )
                            else:
                        ...
                        if intersect[i, j] > thres:
                            if i in region_group:
                                region_group[j] = list(
                                    set(region_group[j] + region_group[i] + [i])
                                )
...
def multiclass_nms_for_cluster(multi_bboxes, multi_scores, labels, nms_thres=0.5):
